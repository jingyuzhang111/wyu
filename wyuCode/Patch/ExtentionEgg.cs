...
public static class ToughEggVisualReplacePatch
{
    private const string TargetMonsterId = "TOUGH_EGG";
    private const string CustomScenePath = "res://wyu/Scenes/creatureVisual/tough_egg_mod.tscn";
    ...
    private static bool Prefix(MonsterModel __instance, ref NCreatureVisuals __result)
    {
        ...
        {
            GD.Print($"[wyu][替换检测] 进入 ToughEgg 补丁，当前怪物ID: {currentId}");
        }

        // 只替换 ToughEgg，其他怪物走原逻辑
        if (!currentId.Equals(TargetMonsterId, System.StringComparison.OrdinalIgnoreCase))
            return true;

        GD.Print($"[wyu][替换命中] ToughEgg 命中，模型类型={__instance.GetType().FullName}，开始替换视觉场景: {CustomScenePath}");

        try
        {
            bool exists = ResourceLoader.Exists(CustomScenePath);
            GD.Print($"[wyu][步骤1] ToughEgg 场景存在性检查: {exists}");

            if (!exists)
            {
                GD.Print($"[wyu][替换失败] 找不到场景文件: {CustomScenePath}，已回退原版 ToughEgg 视觉。");
                return true;
            }

            PackedScene? scene = ResourceLoader.Load<PackedScene>(CustomScenePath);
            GD.Print($"[wyu][步骤2] ToughEgg 场景加载结果: {(scene == null ? "null" : "ok")}");
            if (scene == null)
            {
                GD.Print($"[wyu][替换失败] 场景加载失败: {CustomScenePath}，已回退原版 ToughEgg 视觉。");
                return true; // 回退原版，避免崩
            }

            Node node = scene.Instantiate(PackedScene.GenEditState.Disabled);
            GD.Print($"[wyu][步骤3] ToughEgg 场景实例化类型: C#={node.GetType().FullName}, Godot={node.GetClass()}");

            if (node is NCreatureVisuals visuals)
            {
                GD.Print("[wyu][替换成功] ToughEgg 已切换为自定义视觉场景（原生 NCreatureVisuals）。");
                __result = visuals;
            ...
            if (node is Node2D rawNode)
            {
                ...
                rawNode.QueueFree();
                __result = bridge;
                GD.Print("[wyu][替换成功] ToughEgg 已切换为自定义视觉场景（运行时桥接模式）。");
                return false;
            }

            GD.Print("[wyu][替换失败] 场景根节点既不是 NCreatureVisuals 也不是 Node2D，已回退原版 ToughEgg 视觉。");
            return true;
        }
        catch (Exception ex)
        {
            GD.Print($"[wyu][替换异常] ToughEgg 替换过程中抛异常: {ex}");
            return true;
        }
    }
